...
include 'koneksi.php';
  ...
  if (isset($_GET['id'])) {
    ...
    $data = mysqli_fetch_assoc($result);
      ...
      //  if (!count($data)) {
      if ($data === null) {
        echo "<script>alert('Data not found in database');window.location='your_corp.php';</script>";
       }
  } else {
    // apabila tidak ada data GET id pada akan di redirect ke index.php
    echo "<script>alert('Enter ID data.');window.location='your_corp.php';</script>";
  }         
  ?>
<!DOCTYPE html>
<html>
  ...
  <body>
      ...
      <section class="base">
        ...
        <div>
         <button class="button_tambah" type="submit">Save Changes</button>
        </div>
        </section>
      </form>
